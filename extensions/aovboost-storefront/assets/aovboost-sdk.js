...
`;class G extends h{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},s=y(this.payload),o=String(window.AOVBoost?.currency||"USD"),n=s.reduce((c,f)=>c+Number(f.price||0)*Number(f.quantity||1),0),d=Number(t.discountValue||0),l=t.discountType==="percentage"?n*(1-d/100):t.discountType==="fixed"?Math.max(n-d,0):n;this.html(`
      <style>
      ...
      <section class="bundle card">
        <div class="stack">
          ...
          <div class="tiles">
            ${s.map(c=>`
                  <article class="tile">
                    ${c.imageUrl?`<img src="${a(c.imageUrl)}" alt="${a(c.title)}" loading="lazy">`:""}
                    <p class="product-name">${a(c.title)}</p>
                    <span class="price">${a(c.price?m(c.price,o):"")}</span>
                  </article>
                `).join("")}
          </div>
          <div class="totals">
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_bundle"),await Y(s.map(c=>({variantId:c.variantId,quantity:Number(c.quantity||1)}))),document.dispatchEvent(new CustomEvent("add-to-cart",{detail:{source:"bundle_widget"}}))})}}class Q extends h{constructor(e){super(e);r(this,"messages",[]);r(this,"expanded",!1);const s=e.copy;this.messages.push({role:"assistant",content:String(s?.greeting||e.greeting||"Hi. Can I help you find the perfect product today?")})}getWidgetType(){return"chat"}render(){const e=this.payload.copy||{};this.html(`
      <style>
        .wrap {
          position: fixed;
          left: 18px;
    ...
    `}appendMessage(e){const s=this.root.querySelector("[data-messages]");if(!s)throw new Error("Messages container not found");const o=document.createElement("div");return o.className=`bubble ${e.role}`,o.textContent=e.content,s.appendChild(o),this.scrollToBottom(),o}async sendMessage(){const e=this.root.querySelector("[data-input]"),s=e?.value.trim();if(!s)return;e.value="",this.messages.push({role:"user",content:s}),this.appendMessage({role:"user",content:s}),this.trackClick("send_message");const o=this.messages.push({role:"assistant",content:""})-1,n=this.appendMessage({role:"assistant",content:""});this.showTyping();try{const d=window.AOVBoost||{},l=window.AOVBoostSDK,c=(d.apiBase||"/apps/aovboost").replace(/\/$/,""),f=await fetch(`${c}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-AOVBoost-Shop":l?.shop||d.shop||""},body:JSON.stringify({sessionId:l?.sessionId,shop:l?.shop||d.shop,message:s,messageHistory:this.messages.slice(0,-2)})});if(!f.ok)throw new Error(`Server returned ${f.status}`);if(!f.body)throw new Error("Missing stream body");const dt=f.body.getReader(),ct=new TextDecoder;let S="",k=!1;for(;;){const{done:lt,value:ut}=await dt.read();if(lt)break;S+=ct.decode(ut,{stream:!0});const _=S.split(`
`);S=_.pop()||"";for(const E of _){if(!E.startsWith("data: "))continue;const P=E.slice(6);if(P!=="[DONE]")try{const D=JSON.parse(P);D.delta&&(k||(this.removeTyping(),k=!0),this.messages[o].content+=D.delta,n.textContent=this.messages[o].content,this.updateProductLink(this.messages[o].content,n),this.scrollToBottom())}catch{}}}k||(this.removeTyping(),this.messages[o].content||(this.messages[o].content="I can help you compare products and find the right add-ons.",n.textContent=this.messages[o].content))}catch{this.removeTyping(),this.messages[o].content=this.messages[o].content||"I had trouble connecting. Please try again in a moment.",n.textContent=this.messages[o].content}}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const s=document.createElement("div");s.className="bubble assistant dots",s.dataset.typing="true",s.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(s),this.scrollToBottom()}removeTyping(){const e=this.root.querySelector("[data-typing]");e&&e.remove()}updateProductLink(e,s){const o=e.match(/\/products\/([a-z0-9-]+)/i),n=s.querySelector(".inline-product");if(n&&n.remove(),!o)return;const d=o[1],l=document.createElement("div");l.className="inline-product",l.innerHTML=`
      <p class="product-name">${a(d.replace(/-/g," "))}</p>
      <a href="/products/${a(d)}">View product</a>
    `,s.appendChild(l)}scrollToBottom(){const e=this.root.querySelector("[data-messages]");e&&(e.scrollTop=e.scrollHeight)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateY(0)"},{transform:"translateY(120%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}class Z extends h{getWidgetType(){return"discount_nudge"}render(){this.draw(),document.addEventListener("add-to-cart",()=>this.draw())}draw(){const t=this.payload.copy||{},e=String(window.AOVBoost?.currency||"USD"),s=Number(this.payload.threshold||50),o=Number(this.payload.cartValue||0),n=Math.max(s-o,0),d=s>0?Math.min(o/s,1):0;this.html(`
      <style>
        .bar {
          position: fixed;
          top: 0;
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),n<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class tt extends h{constructor(){super(...arguments);r(this,"shown",!1);r(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});r(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){e.appendChild(this.container),!this.shouldSkip()&&(document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility))}render(){const e=this.payload.copy||{};this.html(`
      <style>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const o=e.variantId;if(o){await v(o);return}const n=e.handle;n&&(window.location.href=`/products/${n}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class st extends h{getWidgetType(){return"rec_strip"}render(){const t=y(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
    ...
    `),this.lazyLoadImages(),this.root.querySelectorAll("[data-add]").forEach(s=>{s.addEventListener("click",async()=>{this.trackClick("add_recommendation"),await v(s.dataset.add)})})}lazyLoadImages(){const t=Array.from(this.root.querySelectorAll("img[data-src]"));if(!("IntersectionObserver"in window)){t.forEach(s=>{s.src=s.dataset.src||""});return}const e=new IntersectionObserver(s=>{s.forEach(o=>{if(!o.isIntersecting)return;const n=o.target;n.src=n.dataset.src||"",e.unobserve(n)})});t.forEach(s=>e.observe(s))}}class it extends h{constructor(){super(...arguments);r(this,"interval")}getWidgetType(){return"social_proof"}render(){const s=y(this.payload).filter(n=>Number(n.orderCount||0)>0).map(n=>`${Number(n.orderCount)} people bought this with ${n.title}`);s.length===0&&s.push("Frequently bought together"),this.html(`
      <style>
        .pill {
          display: inline-flex;
          align-items: center;
    ...
    `);let o=0;this.interval=window.setInterval(()=>{o=(o+1)%s.length;const n=this.root.querySelector("[data-message]");n&&(n.textContent=s[o])},5e3)}destroy(){this.interval&&window.clearInterval(this.interval),super.destroy()}}class ot extends h{constructor(){super(...arguments);r(this,"timer");r(this,"deadline",Date.now()+8e3)}getWidgetType(){return"upsell_drawer"}render(){const e=y(this.payload).slice(0,3),s=this.payload.copy||{},o=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
    ...
    `),this.root.querySelectorAll("[data-dismiss]").forEach(n=>{n.addEventListener("click",()=>this.dismiss())}),this.root.querySelectorAll("[data-add]").forEach(n=>{n.addEventListener("click",async()=>{this.trackClick("add_upsell"),await v(n.dataset.add)})}),this.startCountdown()}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}startCountdown(){this.timer&&window.clearInterval(this.timer),this.deadline=Date.now()+8e3,this.timer=window.setInterval(()=>{const e=Math.max(this.deadline-Date.now(),0),s=this.root.querySelector("[data-timer]");s&&(s.style.transform=`scaleX(${e/8e3})`),e<=0&&this.dismiss()},120)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateX(0)"},{transform:"translateX(100%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}const nt="aovboost_dismissed_widgets";class at{constructor(){r(this,"activeWidget",null);r(this,"activeKey","")}mountDecision(t){if(!t.widgetType||this.getDismissedWidgets().includes(t.widgetType))return;const e=t.payload||{},s=String(e.offerId||""),o=`${t.widgetType}:${s}`;if(o===this.activeKey)return;this.destroyActive();const n=rt(t.widgetType,e);if(!n)return;const d=this.resolveTarget(t.widgetType);n.mount(d),this.activeWidget=n,this.activeKey=o}destroyActive(){this.activeWidget?.destroy(),this.activeWidget=null,this.activeKey=""}getDismissedWidgets(){try{const t=JSON.parse(localStorage.getItem(nt)||"[]");return Array.isArray(t)?t.map(String):[]}catch{return[]}}resolveTarget(t){return t==="bundle"?b(".product-form, [data-product-form]"):t==="rec_strip"?b(".product__description, [data-product-description]"):t==="social_proof"?b(".product-form__submit, [data-add-to-cart]"):document.body}}function rt(i,t){switch(i){case"chat":return new Q(t);case"bundle":return new G(t);case"upsell_drawer":return new ot(t);case"discount_nudge":return new Z(t);case"rec_strip":return new st(t);case"social_proof":return new it(t);case"exit_intent":return new tt(t);case"post_purchase":return new et(t);default:return null}}function b(i){const t=document.querySelector(i),e=document.createElement("div");return e.setAttribute("data-aovboost-mount",i),t?.parentElement?(t.insertAdjacentElement("afterend",e),e):(document.body.appendChild(e),e)}let A=!1;function x(){if(!A){A=!0;try{const i=window.AOVBoost||{},t=i.shop;if(!t)return;const e=i.apiBase||"/apps/aovboost",s=new J(t,e),o=new p({shop:t,sessionManager:s,apiBase:e}),n=new at,d=new F({shop:t,apiBase:e,eventBus:o,sessionManager:s,widgetManager:n});s.init(),o.init(),d.init(),window.AOVBoostSDK={shop:t,sessionId:s.anonymousId,track:(l,c={})=>o.track(l,c),requestOffer:()=>d.requestOffer("global"),destroy:()=>{d.destroy(),s.destroy(),n.destroyActive()}}}catch(i){console.log("AOVBoost SDK skipped:",i instanceof Error?i.message:String(i))}}}return document.readyState==="loading"?document.addEventListener("DOMContentLoaded",x,{once:!0}):x(),u.init=x,Object.defineProperty(u,Symbol.toStringTag,{value:"Module"}),u}({});
