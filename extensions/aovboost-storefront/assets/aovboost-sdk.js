...
`;class H extends h{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},s=f(this.payload),a=String(window.AOVBoost?.currency||"USD"),o=s.reduce((d,y)=>d+Number(y.price||0)*Number(y.quantity||1),0),c=Number(t.discountValue||0),l=t.discountType==="percentage"?o*(1-c/100):t.discountType==="fixed"?Math.max(o-c,0):o;this.html(`
      <style>
    ...
    `}async sendMessage(){const e=this.root.querySelector("[data-input]"),s=e?.value.trim();if(!s)return;e.value="",this.messages.push({role:"user",content:s});const a=this.messages.push({role:"assistant",content:""})-1;this.render(),this.showTyping(),this.trackClick("send_message");try{const o=window.AOVBoostSDK,c=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json","X-AOVBoost-Shop":o?.shop||window.AOVBoost?.shop||""},body:JSON.stringify({sessionId:o?.sessionId,shop:o?.shop||window.AOVBoost?.shop,message:s,messageHistory:this.messages.slice(0,-1)})});if(!c.body)throw new Error("Missing stream body");const l=c.body.getReader(),d=new TextDecoder;let y="";for(;;){const{done:nt,value:rt}=await l.read();if(nt)break;y+=d.decode(rt,{stream:!0});const $=y.split(`
`);y=$.pop()||"";for(const C of $){if(!C.startsWith("data: "))continue;const _=C.slice(6);if(_!=="[DONE]")try{const E=JSON.parse(_);E.delta&&(this.messages[a].content+=E.delta,this.render())}catch{}}}}catch{this.messages[a].content=this.messages[a].content||"I had trouble connecting. Please try again in a moment.",this.render()}}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const s=document.createElement("div");s.className="bubble assistant dots",s.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(s)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateY(0)"},{transform:"translateY(120%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}class G extends h{getWidgetType(){return"discount_nudge"}render(){this.draw(),document.addEventListener("add-to-cart",()=>this.draw())}draw(){const t=this.payload.copy||{},e=String(window.AOVBoost?.currency||"USD"),s=Number(this.payload.threshold||50),a=Number(this.payload.cartValue||0),o=Math.max(s-a,0),c=s>0?Math.min(a/s,1):0;this.html(`
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),o<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class Q extends h{constructor(){super(...arguments);r(this,"shown",!1);r(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});r(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){e.appendChild(this.container),!this.shouldSkip()&&(document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility))}render(){const e=this.payload.copy||{};this.html(`
      <style>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const a=e.variantId;if(a){await v(a);return}const o=e.handle;o&&(window.location.href=`/products/${o}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class tt extends h{getWidgetType(){return"rec_strip"}render(){const t=f(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
    ...
    `),this.lazyLoadImages(),this.root.querySelectorAll("[data-add]").forEach(s=>{s.addEventListener("click",async()=>{this.trackClick("add_recommendation"),await v(s.dataset.add)})})}lazyLoadImages(){const t=Array.from(this.root.querySelectorAll("img[data-src]"));if(!("IntersectionObserver"in window)){t.forEach(s=>{s.src=s.dataset.src||""});return}const e=new IntersectionObserver(s=>{s.forEach(a=>{if(!a.isIntersecting)return;const o=a.target;o.src=o.dataset.src||"",e.unobserve(o)})});t.forEach(s=>e.observe(s))}}class et extends h{constructor(){super(...arguments);r(this,"interval")}getWidgetType(){return"social_proof"}render(){const s=f(this.payload).filter(o=>Number(o.orderCount||0)>0).map(o=>`${Number(o.orderCount)} people bought this with ${o.title}`);s.length===0&&s.push("Frequently bought together"),this.html(`
      <style>
        .pill {
          display: inline-flex;
          align-items: center;
