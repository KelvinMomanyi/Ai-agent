...
`;class _t extends h{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},s=v(this.payload),o=String(window.AOVBoost?.currency||"USD"),n=s.length>0&&s.every(l=>l.variantId),d=s.find(l=>l.handle)?.handle,c=s.reduce((l,b)=>l+Number(b.price||0)*Number(b.quantity||1),0),u=Number(t.discountValue||0),p=t.discountType==="percentage"?c*(1-u/100):t.discountType==="fixed"?Math.max(c-u,0):c;this.html(`
      <style>
      ...
      <section class="bundle card">
        <div class="stack">
          ...
          <div class="totals">
            ${c>p?`<span class="strike">${y(c,o)}</span>`:""}
            <strong>${y(p,o)}</strong>
          </div>
          <div class="actions">
            ${n?`<button type="button" class="primary" data-add>${r(e.ctaText||"Add bundle to cart")}</button>`:d?`<a class="primary" href="/products/${r(d)}">${r(e.ctaText||"View bundle products")}</a>`:""}
          </div>
        </div>
      </section>
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_bundle"),await wt(s.map(l=>({variantId:l.variantId,quantity:Number(l.quantity||1)}))),document.dispatchEvent(new CustomEvent("add-to-cart",{detail:{source:"bundle_widget"}}))})}}class St extends h{constructor(e){super(e);a(this,"messages",[]);a(this,"expanded",!1);a(this,"sending",!1);const s=e.copy;this.messages.push({role:"assistant",content:String(s?.greeting||e.greeting||"Hi. Can I help you find the perfect product today?")})}getWidgetType(){return"chat"}render(){const e=this.payload.copy||{};this.html(`
      <style>
        .wrap {
          position: fixed;
          left: 18px;
...
`);l=F.pop()||"";for(const U of F){if(!U.startsWith("data: "))continue;const z=U.slice(6);if(z!=="[DONE]")try{const R=JSON.parse(z);R.delta&&(b||(this.removeTyping(),b=!0),this.messages[n].content+=R.delta,d.textContent=this.messages[n].content,this.updateProductLink(this.messages[n].content,d),this.scrollToBottom())}catch{}}}b||(this.removeTyping(),this.messages[n].content||(this.messages[n].content="I can help you compare products and find the right add-ons.",d.textContent=this.messages[n].content))}catch{this.removeTyping(),this.messages[n].content=this.messages[n].content||"I had trouble connecting. Please try again in a moment.",d.textContent=this.messages[n].content}finally{this.sending=!1,s&&(s.disabled=!1)}}requestChat(e){const s=window.AOVBoost||{},o=window.AOVBoostSDK,n=xt(s.apiBase).replace(/\/$/,"");return fetch(`${n}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-AOVBoost-Shop":o?.shop||s.shop||""},body:JSON.stringify({sessionId:o?.sessionId,sessionToken:o?.sessionToken,shop:o?.shop||s.shop,message:e,messageHistory:this.messages.slice(0,-2)})})}async applyRecoverySession(e){try{const s=await e.clone().json(),o=s?.storefrontSession||s?.session,n=window.AOVBoostSDK?.applySession;return typeof n=="function"?!!n(o):!1}catch{return!1}}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const s=document.createElement("div");s.className="bubble assistant dots",s.dataset.typing="true",s.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(s),this.scrollToBottom()}removeTyping(){const e=this.root.querySelector("[data-typing]");e&&e.remove()}updateProductLink(e,s){const o=e.match(/\/products\/([a-z0-9-]+)/i),n=s.querySelector(".inline-product");if(n&&n.remove(),!o)return;const d=o[1],c=document.createElement("div");c.className="inline-product",c.innerHTML=`
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),this.tick(),this.timer=window.setInterval(()=>this.tick(),1e3)}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}tick(){const e=this.root.querySelector("[data-countdown]");if(!e)return;const s=Date.parse(String(this.payload.endsAt||""));if(!Number.isFinite(s)){e.textContent="Today";return}const o=Math.max(s-Date.now(),0);if(o<=0){this.destroy();return}const n=Math.floor(o/36e5),d=Math.floor(o%36e5/6e4),c=Math.floor(o%6e4/1e3);e.textContent=n>0?`${n}h ${d}m`:`${d}m ${c.toString().padStart(2,"0")}s`}}class It extends h{getWidgetType(){return"discount_nudge"}render(){this.draw(),document.addEventListener("add-to-cart",()=>this.draw())}draw(){const t=this.payload.copy||{},e=String(window.AOVBoost?.currency||"USD"),s=Number(this.payload.threshold||50),o=Number(this.payload.cartValue||0),n=Math.max(s-o,0),d=s>0?Math.min(o/s,1):0;this.html(`
      ...
      <div class="bar">
        <div class="label">
          <span>${n>0?r(t.progressLabel||`You're ${y(n,e)} away from your reward`):r(t.rewardDescription||"Reward unlocked")}</span>
          <button type="button" class="icon" data-dismiss aria-label="Close">x</button>
        </div>
        <div class="track" aria-hidden="true"><span></span></div>
      </div>
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),n<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class At extends h{constructor(){super(...arguments);a(this,"shown",!1);a(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});a(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){if(e.appendChild(this.container),!this.shouldSkip()){if(this.payload.immediate){this.show();return}document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility)}}render(){const e=this.payload.copy||{};this.html(`
      <style>
        ...
        <article class="product-card">
          ...
          <div class="stack">
            <div>
              ...
              <span class="price">${r(e.price?y(e.price,s):"")}</span>
            </div>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const o=e.variantId;if(o){await C(o);return}const n=e.handle;n&&(window.location.href=`/products/${n}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class $t extends h{getWidgetType(){return"rec_strip"}render(){const t=v(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
        ...
        <div class="rail">
          ${t.map(s=>`
                <article class="tile">
                  ...
                  <span class="price">${r(s.price?y(s.price,e):"")}</span>
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),this.root.querySelector("[data-chat]")?.addEventListener("click",()=>{this.trackClick("open_assistant"),document.dispatchEvent(new CustomEvent("aovboost:trigger",{detail:{type:"long_product_dwell",source:"toast"}})),this.destroy()}),window.setTimeout(()=>this.destroy(),9e3)}}class Dt extends h{constructor(){super(...arguments);a(this,"timer");a(this,"deadline",Date.now()+8e3)}getWidgetType(){return"upsell_drawer"}render(){const e=v(this.payload).slice(0,3),s=this.payload.copy||{},o=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
                ...
                <article class="product-card">
                  ...
                  <div class="stack">
                    <div>
                      ...
                      <span class="price">${r(n.price?y(n.price,o):"")}</span>
                    </div>
