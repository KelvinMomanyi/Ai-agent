...
`;class X extends h{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},s=y(this.payload),a=String(window.AOVBoost?.currency||"USD"),o=s.reduce((d,f)=>d+Number(f.price||0)*Number(f.quantity||1),0),c=Number(t.discountValue||0),l=t.discountType==="percentage"?o*(1-c/100):t.discountType==="fixed"?Math.max(o-c,0):o;this.html(`
      <style>
      ...
      <section class="bundle card">
        <div class="stack">
          ...
          <div class="tiles">
            ${s.map(d=>`
                  <article class="tile">
                    ${d.imageUrl?`<img src="${n(d.imageUrl)}" alt="${n(d.title)}" loading="lazy">`:""}
                    <p class="product-name">${n(d.title)}</p>
                    <span class="price">${n(d.price?m(d.price,a):"")}</span>
                  </article>
                `).join("")}
          </div>
          <div class="totals">
            ${o>l?`<span class="strike">${m(o,a)}</span>`:""}
            <strong>${m(l,a)}</strong>
          </div>
          <div class="actions">
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_bundle"),await Y(s.map(d=>({variantId:d.variantId,quantity:Number(d.quantity||1)}))),document.dispatchEvent(new CustomEvent("add-to-cart",{detail:{source:"bundle_widget"}}))})}}class G extends h{constructor(e){super(e);r(this,"messages",[]);r(this,"expanded",!1);const s=e.copy;this.messages.push({role:"assistant",content:String(s?.greeting||e.greeting||"Hi. Can I help you find the perfect product today?")})}getWidgetType(){return"chat"}render(){const e=this.payload.copy||{};this.html(`
      <style>
        .wrap {
          position: fixed;
          left: 18px;
    ...
    `}renderProductLinks(e){const s=e.match(/\/products\/([a-z0-9-]+)/i);if(!s)return"";const a=s[1];return`
      <div class="inline-product">
        <p class="product-name">${n(a.replace(/-/g," "))}</p>
        <a href="/products/${n(a)}">View product</a>
      </div>
    `}async sendMessage(){const e=this.root.querySelector("[data-input]"),s=e?.value.trim();if(!s)return;e.value="",this.messages.push({role:"user",content:s});const a=this.messages.push({role:"assistant",content:""})-1;this.render(),this.showTyping(),this.trackClick("send_message");try{const o=window.AOVBoost||{},c=window.AOVBoostSDK,l=(o.apiBase||"/apps/aovboost").replace(/\/$/,""),d=await fetch(`${l}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-AOVBoost-Shop":c?.shop||o.shop||""},body:JSON.stringify({sessionId:c?.sessionId,shop:c?.shop||o.shop,message:s,messageHistory:this.messages.slice(0,-1)})});if(!d.body)throw new Error("Missing stream body");const f=d.body.getReader(),rt=new TextDecoder;let S="";for(;;){const{done:dt,value:ct}=await f.read();if(dt)break;S+=rt.decode(ct,{stream:!0});const C=S.split(`
`);S=C.pop()||"";for(const _ of C){if(!_.startsWith("data: "))continue;const E=_.slice(6);if(E!=="[DONE]")try{const P=JSON.parse(E);P.delta&&(this.messages[a].content+=P.delta,this.render())}catch{}}}}catch{this.messages[a].content=this.messages[a].content||"I had trouble connecting. Please try again in a moment.",this.render()}}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const s=document.createElement("div");s.className="bubble assistant dots",s.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(s)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateY(0)"},{transform:"translateY(120%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}class Q extends h{getWidgetType(){return"discount_nudge"}render(){this.draw(),document.addEventListener("add-to-cart",()=>this.draw())}draw(){const t=this.payload.copy||{},e=String(window.AOVBoost?.currency||"USD"),s=Number(this.payload.threshold||50),a=Number(this.payload.cartValue||0),o=Math.max(s-a,0),c=s>0?Math.min(a/s,1):0;this.html(`
      <style>
        .bar {
          position: fixed;
          top: 0;
      ...
      <div class="bar">
        <div class="label">
          <span>${o>0?n(t.progressLabel||`You're ${m(o,e)} away from your reward`):n(t.rewardDescription||"Reward unlocked")}</span>
          <button type="button" class="icon" data-dismiss aria-label="Close">x</button>
        </div>
        <div class="track" aria-hidden="true"><span></span></div>
      </div>
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),o<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class Z extends h{constructor(){super(...arguments);r(this,"shown",!1);r(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});r(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){e.appendChild(this.container),!this.shouldSkip()&&(document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility))}render(){const e=this.payload.copy||{};this.html(`
      <style>
        ...
        <article class="product-card">
          ...
          <div class="stack">
            <div>
              ...
              <span class="price">${n(e.price?m(e.price,s):"")}</span>
            </div>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const a=e.variantId;if(a){await v(a);return}const o=e.handle;o&&(window.location.href=`/products/${o}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class et extends h{getWidgetType(){return"rec_strip"}render(){const t=y(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
        ...
        <div class="rail">
          ${t.map(s=>`
                <article class="tile">
                  ...
                  <span class="price">${n(s.price?m(s.price,e):"")}</span>
                  <button type="button" class="primary" data-add="${n(s.variantId)}">Add to cart</button>
                </article>
              `).join("")}
        </div>
      </section>
    `),this.lazyLoadImages(),this.root.querySelectorAll("[data-add]").forEach(s=>{s.addEventListener("click",async()=>{this.trackClick("add_recommendation"),await v(s.dataset.add)})})}lazyLoadImages(){const t=Array.from(this.root.querySelectorAll("img[data-src]"));if(!("IntersectionObserver"in window)){t.forEach(s=>{s.src=s.dataset.src||""});return}const e=new IntersectionObserver(s=>{s.forEach(a=>{if(!a.isIntersecting)return;const o=a.target;o.src=o.dataset.src||"",e.unobserve(o)})});t.forEach(s=>e.observe(s))}}class st extends h{constructor(){super(...arguments);r(this,"interval")}getWidgetType(){return"social_proof"}render(){const s=y(this.payload).filter(o=>Number(o.orderCount||0)>0).map(o=>`${Number(o.orderCount)} people bought this with ${o.title}`);s.length===0&&s.push("Frequently bought together"),this.html(`
      <style>
        .pill {
          display: inline-flex;
          align-items: center;
    ...
    `);let a=0;this.interval=window.setInterval(()=>{a=(a+1)%s.length;const o=this.root.querySelector("[data-message]");o&&(o.textContent=s[a])},5e3)}destroy(){this.interval&&window.clearInterval(this.interval),super.destroy()}}class it extends h{constructor(){super(...arguments);r(this,"timer");r(this,"deadline",Date.now()+8e3)}getWidgetType(){return"upsell_drawer"}render(){const e=y(this.payload).slice(0,3),s=this.payload.copy||{},a=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
      ...
      <aside class="drawer" aria-label="Add-to-cart upsell">
        ...
        <div class="product-grid">
          ${e.map(o=>`
                <article class="product-card">
                  ${o.imageUrl?`<img src="${n(o.imageUrl)}" alt="${n(o.title)}" loading="lazy">`:"<span></span>"}
                  <div class="stack">
                    <div>
                      <p class="product-name">${n(o.title)}</p>
                      <span class="price">${n(o.price?m(o.price,a):"")}</span>
                    </div>
                    <p class="reason">${n(o.reason||s.whyThisGoes||"It pairs well with your cart.")}</p>
                    <button type="button" class="primary" data-add="${n(o.variantId)}">Add to cart</button>
                  </div>
                </article>
              `).join("")}
        </div>
    ...
    `),this.root.querySelectorAll("[data-dismiss]").forEach(o=>{o.addEventListener("click",()=>this.dismiss())}),this.root.querySelectorAll("[data-add]").forEach(o=>{o.addEventListener("click",async()=>{this.trackClick("add_upsell"),await v(o.dataset.add)})}),this.startCountdown()}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}startCountdown(){this.timer&&window.clearInterval(this.timer),this.deadline=Date.now()+8e3,this.timer=window.setInterval(()=>{const e=Math.max(this.deadline-Date.now(),0),s=this.root.querySelector("[data-timer]");s&&(s.style.transform=`scaleX(${e/8e3})`),e<=0&&this.dismiss()},120)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateX(0)"},{transform:"translateX(100%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}const at="aovboost_dismissed_widgets";class ot{constructor(){r(this,"activeWidget",null);r(this,"activeKey","")}mountDecision(t){if(!t.widgetType||this.getDismissedWidgets().includes(t.widgetType))return;const e=t.payload||{},s=String(e.offerId||""),a=`${t.widgetType}:${s}`;if(a===this.activeKey)return;this.destroyActive();const o=nt(t.widgetType,e);if(!o)return;const c=this.resolveTarget(t.widgetType);o.mount(c),this.activeWidget=o,this.activeKey=a}destroyActive(){this.activeWidget?.destroy(),this.activeWidget=null,this.activeKey=""}getDismissedWidgets(){try{const t=JSON.parse(localStorage.getItem(at)||"[]");return Array.isArray(t)?t.map(String):[]}catch{return[]}}resolveTarget(t){return t==="bundle"?b(".product-form, [data-product-form]"):t==="rec_strip"?b(".product__description, [data-product-description]"):t==="social_proof"?b(".product-form__submit, [data-add-to-cart]"):document.body}}function nt(i,t){switch(i){case"chat":return new G(t);case"bundle":return new X(t);case"upsell_drawer":return new it(t);case"discount_nudge":return new Q(t);case"rec_strip":return new et(t);case"social_proof":return new st(t);case"exit_intent":return new Z(t);case"post_purchase":return new tt(t);default:return null}}function b(i){const t=document.querySelector(i),e=document.createElement("div");return e.setAttribute("data-aovboost-mount",i),t?.parentElement?(t.insertAdjacentElement("afterend",e),e):(document.body.appendChild(e),e)}let T=!1;function x(){if(!T){T=!0;try{const i=window.AOVBoost||{},t=i.shop;if(!t)return;const e=i.apiBase||"/apps/aovboost",s=new J(t,e),a=new p({shop:t,sessionManager:s,apiBase:e}),o=new ot,c=new z({shop:t,apiBase:e,eventBus:a,sessionManager:s,widgetManager:o});s.init(),a.init(),c.init(),window.AOVBoostSDK={shop:t,sessionId:s.anonymousId,track:(l,d={})=>a.track(l,d),requestOffer:()=>c.requestOffer("global"),destroy:()=>{c.destroy(),s.destroy(),o.destroyActive()}}}catch(i){console.log("AOVBoost SDK skipped:",i instanceof Error?i.message:String(i))}}}return document.readyState==="loading"?document.addEventListener("DOMContentLoaded",x,{once:!0}):x(),u.init=x,Object.defineProperty(u,Symbol.toStringTag,{value:"Module"}),u}({});
