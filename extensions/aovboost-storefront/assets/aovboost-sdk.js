...
`;class Z extends h{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},s=f(this.payload),o=String(window.AOVBoost?.currency||"USD"),n=s.reduce((c,y)=>c+Number(y.price||0)*Number(y.quantity||1),0),d=Number(t.discountValue||0),l=t.discountType==="percentage"?n*(1-d/100):t.discountType==="fixed"?Math.max(n-d,0):n;this.html(`
      <style>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_bundle"),await G(s.map(c=>({variantId:c.variantId,quantity:Number(c.quantity||1)}))),document.dispatchEvent(new CustomEvent("add-to-cart",{detail:{source:"bundle_widget"}}))})}}class tt extends h{constructor(e){super(e);r(this,"messages",[]);r(this,"expanded",!1);r(this,"sending",!1);const s=e.copy;this.messages.push({role:"assistant",content:String(s?.greeting||e.greeting||"Hi. Can I help you find the perfect product today?")})}getWidgetType(){return"chat"}render(){const e=this.payload.copy||{};this.html(`
      <style>
        .wrap {
          position: fixed;
          left: 18px;
...
`);k=E.pop()||"";for(const P of E){if(!P.startsWith("data: "))continue;const D=P.slice(6);if(D!=="[DONE]")try{const q=JSON.parse(D);q.delta&&(T||(this.removeTyping(),T=!0),this.messages[n].content+=q.delta,d.textContent=this.messages[n].content,this.updateProductLink(this.messages[n].content,d),this.scrollToBottom())}catch{}}}T||(this.removeTyping(),this.messages[n].content||(this.messages[n].content="I can help you compare products and find the right add-ons.",d.textContent=this.messages[n].content))}catch{this.removeTyping(),this.messages[n].content=this.messages[n].content||"I had trouble connecting. Please try again in a moment.",d.textContent=this.messages[n].content}finally{this.sending=!1,s&&(s.disabled=!1)}}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const s=document.createElement("div");s.className="bubble assistant dots",s.dataset.typing="true",s.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(s),this.scrollToBottom()}removeTyping(){const e=this.root.querySelector("[data-typing]");e&&e.remove()}updateProductLink(e,s){const o=e.match(/\/products\/([a-z0-9-]+)/i),n=s.querySelector(".inline-product");if(n&&n.remove(),!o)return;const d=o[1],l=document.createElement("div");l.className="inline-product",l.innerHTML=`
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),n<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class st extends h{constructor(){super(...arguments);r(this,"shown",!1);r(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});r(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){e.appendChild(this.container),!this.shouldSkip()&&(document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility))}render(){const e=this.payload.copy||{};this.html(`
      <style>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const o=e.variantId;if(o){await b(o);return}const n=e.handle;n&&(window.location.href=`/products/${n}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class ot extends h{getWidgetType(){return"rec_strip"}render(){const t=f(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
    ...
    `),this.root.querySelectorAll("[data-dismiss]").forEach(n=>{n.addEventListener("click",()=>this.dismiss())}),this.root.querySelectorAll("[data-add]").forEach(n=>{n.addEventListener("click",async()=>{this.trackClick("add_upsell"),await b(n.dataset.add)})}),this.startCountdown()}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}startCountdown(){this.timer&&window.clearInterval(this.timer),this.deadline=Date.now()+8e3,this.timer=window.setInterval(()=>{const e=Math.max(this.deadline-Date.now(),0),s=this.root.querySelector("[data-timer]");s&&(s.style.transform=`scaleX(${e/8e3})`),e<=0&&this.dismiss()},120)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateX(0)"},{transform:"translateX(100%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}const rt="aovboost_dismissed_widgets";class ct{constructor(){r(this,"activeWidget",null);r(this,"activeKey","");r(this,"activeWidgetType","")}mountDecision(t){if(!t.widgetType||this.getDismissedWidgets().includes(t.widgetType))return;const e=t.payload||{},s=String(e.offerId||""),o=`${t.widgetType}:${s}`;if(t.widgetType==="chat"&&this.activeWidgetType==="chat"||o===this.activeKey)return;this.destroyActive();const n=dt(t.widgetType,e);if(!n)return;const d=this.resolveTarget(t.widgetType);n.mount(d),this.activeWidget=n,this.activeKey=o,this.activeWidgetType=t.widgetType}destroyActive(){this.activeWidget?.destroy(),this.activeWidget=null,this.activeKey="",this.activeWidgetType=""}getDismissedWidgets(){try{const t=JSON.parse(localStorage.getItem(rt)||"[]");return Array.isArray(t)?t.map(String):[]}catch{return[]}}resolveTarget(t){return t==="bundle"?x(".product-form, [data-product-form]"):t==="rec_strip"?x(".product__description, [data-product-description]"):t==="social_proof"?x(".product-form__submit, [data-add-to-cart]"):document.body}}function dt(i,t){switch(i){case"chat":return new tt(t);case"bundle":return new Z(t);case"upsell_drawer":return new at(t);case"discount_nudge":return new et(t);case"rec_strip":return new ot(t);case"social_proof":return new nt(t);case"exit_intent":return new st(t);case"post_purchase":return new it(t);default:return null}}function x(i){const t=document.querySelector(i),e=document.createElement("div");return e.setAttribute("data-aovboost-mount",i),t?.parentElement?(t.insertAdjacentElement("afterend",e),e):(document.body.appendChild(e),e)}let $=!1;function S(){$||($=!0,lt().catch(i=>{console.log("AOVBoost SDK skipped:",i instanceof Error?i.message:String(i))}))}async function lt(){try{const i=window.AOVBoost||{},t=i.shop;if(!t)return;_(i)||await ut(i);const e=i.apiBase||"/apps/aovboost",s=new H(t,e),o=new p({shop:t,sessionManager:s,apiBase:e}),n=new ct,d=new R({shop:t,apiBase:e,eventBus:o,sessionManager:s,widgetManager:n});await s.init(),o.init(),d.init(),window.AOVBoostSDK={shop:t,sessionId:s.anonymousId,sessionToken:s.getAuthPayload().sessionToken,track:(l,c={})=>o.track(l,c),requestOffer:()=>d.requestOffer("global"),destroy:()=>{d.destroy(),s.destroy(),n.destroyActive()}}}catch(i){console.log("AOVBoost SDK skipped:",i instanceof Error?i.message:String(i))}}function _(i){const t=window.Shopify?.customerPrivacy;return t?.analyticsProcessingAllowed instanceof Function?!!t.analyticsProcessingAllowed():t?.userCanBeTracked instanceof Function?!!t.userCanBeTracked():i.settings?.trackingConsentRequired!==!0}function ut(i){return new Promise(t=>{const e=()=>{_({...i,settings:{...i.settings,trackingConsentRequired:!1}})&&(s(),t())},s=()=>{["visitorConsentCollected","shopify:customer_privacy:consent_collected","aovboost:consent-granted"].forEach(o=>window.removeEventListener(o,e))};["visitorConsentCollected","shopify:customer_privacy:consent_collected","aovboost:consent-granted"].forEach(o=>window.addEventListener(o,e))})}return document.readyState==="loading"?document.addEventListener("DOMContentLoaded",S,{once:!0}):S(),u.init=S,Object.defineProperty(u,Symbol.toStringTag,{value:"Module"}),u}({});
