...
`;class wt extends p{getWidgetType(){return"bundle"}render(){const t=this.payload.bundle||{},e=this.payload.copy||{},i=v(this.payload),o=String(window.AOVBoost?.currency||"USD"),a=i.length>0&&i.every(l=>l.variantId),d=i.find(l=>l.handle)?.handle,c=i.reduce((l,b)=>l+Number(b.price||0)*Number(b.quantity||1),0),u=Number(t.discountValue||0),h=t.discountType==="percentage"?c*(1-u/100):t.discountType==="fixed"?Math.max(c-u,0):c;this.html(`
      <style>
          ...
          <div class="tiles">
            ${i.map(l=>`
                  <article class="tile">
                    ...
                    <span class="price">${r(l.price?y(l.price,o):"")}</span>
                  </article>
                `).join("")}
          </div>
          <div class="totals">
            ${c>h?`<span class="strike">${y(c,o)}</span>`:""}
            <strong>${y(h,o)}</strong>
          </div>
          <div class="actions">
            ${a?`<button type="button" class="primary" data-add>${r(e.ctaText||"Add bundle to cart")}</button>`:d?`<a class="primary" href="/products/${r(d)}">${r(e.ctaText||"View bundle products")}</a>`:""}
          </div>
        </div>
      </section>
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_bundle"),await mt(i.map(l=>({variantId:l.variantId,quantity:Number(l.quantity||1)}))),document.dispatchEvent(new CustomEvent("add-to-cart",{detail:{source:"bundle_widget"}}))})}}class bt extends p{constructor(e){super(e);n(this,"messages",[]);n(this,"expanded",!1);n(this,"sending",!1);const i=e.copy;this.messages.push({role:"assistant",content:String(i?.greeting||e.greeting||"Hi. Can I help you find the perfect product today?")})}getWidgetType(){return"chat"}render(){const e=this.payload.copy||{};this.html(`
      <style>
        .wrap {
          position: fixed;
          left: 18px;
...
`);l=H.pop()||"";for(const W of H){if(!W.startsWith("data: "))continue;const F=W.slice(6);if(F!=="[DONE]")try{const U=JSON.parse(F);U.delta&&(b||(this.removeTyping(),b=!0),this.messages[a].content+=U.delta,d.textContent=this.messages[a].content,this.updateProductLink(this.messages[a].content,d),this.scrollToBottom())}catch{}}}b||(this.removeTyping(),this.messages[a].content||(this.messages[a].content="I can help you compare products and find the right add-ons.",d.textContent=this.messages[a].content))}catch{this.removeTyping(),this.messages[a].content=this.messages[a].content||"I had trouble connecting. Please try again in a moment.",d.textContent=this.messages[a].content}finally{this.sending=!1,i&&(i.disabled=!1)}}requestChat(e){const i=window.AOVBoost||{},o=window.AOVBoostSDK,a=_t(i.apiBase).replace(/\/$/,"");return fetch(`${a}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-AOVBoost-Shop":o?.shop||i.shop||""},body:JSON.stringify({sessionId:o?.sessionId,sessionToken:o?.sessionToken,shop:o?.shop||i.shop,message:e,messageHistory:this.messages.slice(0,-2)})})}showTyping(){const e=this.root.querySelector("[data-messages]");if(!e)return;const i=document.createElement("div");i.className="bubble assistant dots",i.dataset.typing="true",i.innerHTML="<span>.</span><span>.</span><span>.</span>",e.appendChild(i),this.scrollToBottom()}removeTyping(){const e=this.root.querySelector("[data-typing]");e&&e.remove()}updateProductLink(e,i){const o=e.match(/\/products\/([a-z0-9-]+)/i),a=i.querySelector(".inline-product");if(a&&a.remove(),!o)return;const d=o[1],c=document.createElement("div");c.className="inline-product",c.innerHTML=`
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),this.tick(),this.timer=window.setInterval(()=>this.tick(),1e3)}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}tick(){const e=this.root.querySelector("[data-countdown]");if(!e)return;const i=Date.parse(String(this.payload.endsAt||""));if(!Number.isFinite(i)){e.textContent="Today";return}const o=Math.max(i-Date.now(),0);if(o<=0){this.destroy();return}const a=Math.floor(o/36e5),d=Math.floor(o%36e5/6e4),c=Math.floor(o%6e4/1e3);e.textContent=a>0?`${a}h ${d}m`:`${d}m ${c.toString().padStart(2,"0")}s`}}class kt extends p{getWidgetType(){return"discount_nudge"}render(){this.draw(),document.addEventListener("add-to-cart",()=>this.draw())}draw(){const t=this.payload.copy||{},e=String(window.AOVBoost?.currency||"USD"),i=Number(this.payload.threshold||50),o=Number(this.payload.cartValue||0),a=Math.max(i-o,0),d=i>0?Math.min(o/i,1):0;this.html(`
      ...
      <div class="bar">
        <div class="label">
          <span>${a>0?r(t.progressLabel||`You're ${y(a,e)} away from your reward`):r(t.rewardDescription||"Reward unlocked")}</span>
          <button type="button" class="icon" data-dismiss aria-label="Close">x</button>
        </div>
        <div class="track" aria-hidden="true"><span></span></div>
      </div>
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),a<=0&&window.setTimeout(()=>this.destroy(),3e3)}}class xt extends p{constructor(){super(...arguments);n(this,"shown",!1);n(this,"handleMouseLeave",e=>{e.clientY<10&&this.show()});n(this,"handleVisibility",()=>{document.visibilityState==="hidden"&&this.show()})}getWidgetType(){return"exit_intent"}mount(e=document.body){if(e.appendChild(this.container),!this.shouldSkip()){if(this.payload.immediate){this.show();return}document.addEventListener("mouseleave",this.handleMouseLeave),document.addEventListener("visibilitychange",this.handleVisibility)}}render(){const e=this.payload.copy||{};this.html(`
      <style>
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()})}}class It extends p{getWidgetType(){return"post_purchase"}mount(t=document.body){this.isThankYouPage()&&super.mount(t)}render(){const t=this.payload.copy||{},e=v(this.payload)[0]||this.payload.product||{},i=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
        ...
        <article class="product-card">
          ...
          <div class="stack">
            <div>
              ...
              <span class="price">${r(e.price?y(e.price,i):"")}</span>
            </div>
    ...
    `),this.root.querySelector("[data-add]")?.addEventListener("click",async()=>{this.trackClick("add_post_purchase");const o=e.variantId;if(o){await A(o);return}const a=e.handle;a&&(window.location.href=`/products/${a}`)})}isThankYouPage(){return/\/thank_you(?:\/|$)/.test(window.location.pathname)||!!window.Shopify?.checkout}}class Ct extends p{getWidgetType(){return"rec_strip"}render(){const t=v(this.payload),e=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
        ...
        <div class="rail">
          ${t.map(i=>`
                <article class="tile">
                  ...
                  <span class="price">${r(i.price?y(i.price,e):"")}</span>
                  ${i.variantId?`<button type="button" class="primary" data-add="${r(i.variantId)}">Add to cart</button>`:i.handle?`<a class="primary" href="/products/${r(i.handle)}">View product</a>`:""}
                </article>
              `).join("")}
        </div>
      </section>
    `),this.lazyLoadImages(),this.root.querySelectorAll("[data-add]").forEach(i=>{i.addEventListener("click",async()=>{this.trackClick("add_recommendation"),await A(i.dataset.add)})})}lazyLoadImages(){const t=Array.from(this.root.querySelectorAll("img[data-src]"));if(!("IntersectionObserver"in window)){t.forEach(i=>{i.src=i.dataset.src||""});return}const e=new IntersectionObserver(i=>{i.forEach(o=>{if(!o.isIntersecting)return;const a=o.target;a.src=a.dataset.src||"",e.unobserve(a)})});t.forEach(i=>e.observe(i))}}class At extends p{constructor(){super(...arguments);n(this,"interval")}getWidgetType(){return"social_proof"}render(){const i=v(this.payload).filter(a=>Number(a.orderCount||0)>0).map(a=>`${Number(a.orderCount)} people bought this with ${a.title}`);i.length===0&&i.push("Frequently bought together"),this.html(`
      <style>
        .pill {
          display: inline-flex;
          align-items: center;
    ...
    `),this.root.querySelector("[data-dismiss]")?.addEventListener("click",()=>{this.trackDismiss(),this.destroy()}),this.root.querySelector("[data-chat]")?.addEventListener("click",()=>{this.trackClick("open_assistant"),document.dispatchEvent(new CustomEvent("aovboost:trigger",{detail:{type:"long_product_dwell",source:"toast"}})),this.destroy()}),window.setTimeout(()=>this.destroy(),9e3)}}class $t extends p{constructor(){super(...arguments);n(this,"timer");n(this,"deadline",Date.now()+8e3)}getWidgetType(){return"upsell_drawer"}render(){const e=v(this.payload).slice(0,3),i=this.payload.copy||{},o=String(window.AOVBoost?.currency||"USD");this.html(`
      <style>
                ...
                <article class="product-card">
                  ...
                  <div class="stack">
                    <div>
                      ...
                      <span class="price">${r(a.price?y(a.price,o):"")}</span>
                    </div>
                    <p class="reason">${r(a.reason||i.whyThisGoes||"It pairs well with your cart.")}</p>
                    ${a.variantId?`<button type="button" class="primary" data-add="${r(a.variantId)}">Add to cart</button>`:a.handle?`<a class="primary" href="/products/${r(a.handle)}">View product</a>`:""}
                  </div>
                </article>
              `).join("")}
        </div>
    ...
    `),this.root.querySelectorAll("[data-dismiss]").forEach(a=>{a.addEventListener("click",()=>this.dismiss())}),this.root.querySelectorAll("[data-add]").forEach(a=>{a.addEventListener("click",async()=>{this.trackClick("add_upsell"),await A(a.dataset.add)})}),this.startCountdown()}destroy(){this.timer&&window.clearInterval(this.timer),super.destroy()}startCountdown(){this.timer&&window.clearInterval(this.timer),this.deadline=Date.now()+8e3,this.timer=window.setInterval(()=>{const e=Math.max(this.deadline-Date.now(),0),i=this.root.querySelector("[data-timer]");i&&(i.style.transform=`scaleX(${e/8e3})`),e<=0&&this.dismiss()},120)}dismiss(){this.trackDismiss(),this.container.animate([{transform:"translateX(0)"},{transform:"translateX(100%)"}],{duration:180,easing:"ease-in",fill:"forwards"}),window.setTimeout(()=>this.destroy(),190)}}const B="aovboost_dismissed_widgets",Et=30*60*1e3;class Lt{constructor(){n(this,"activeWidget",null);n(this,"activeKey","");n(this,"activeWidgetType","")}mountDecision(t){if(!t.widgetType||this.getDismissedWidgets().includes(t.widgetType))return;const e=t.payload||{},i=String(e.offerId||""),o=`${t.widgetType}:${i}`;if(t.widgetType==="chat"&&this.activeWidgetType==="chat"||o===this.activeKey)return;this.destroyActive();const a=Dt(t.widgetType,e);if(!a)return;const d=this.resolveTarget(t.widgetType);a.mount(d),this.activeWidget=a,this.activeKey=o,this.activeWidgetType=t.widgetType}destroyActive(){this.activeWidget?.destroy(),this.activeWidget=null,this.activeKey="",this.activeWidgetType=""}getDismissedWidgets(){try{const t=JSON.parse(localStorage.getItem(B)||"[]");if(!Array.isArray(t))return[];const e=Date.now(),i=t.filter(o=>o&&typeof o=="object").filter(o=>e-Number(o.dismissedAt||0)<Et);return i.length!==t.length&&localStorage.setItem(B,JSON.stringify(i)),i.map(o=>String(o.widgetType||"")).filter(Boolean)}catch{return[]}}resolveTarget(t){return t==="bundle"?S(".product-form, [data-product-form]"):t==="rec_strip"?S(".product__description, [data-product-description]"):t==="social_proof"?S(".product-form__submit, [data-add-to-cart]"):t==="inline_alert"?S("[data-price], .product__price, .price, .product-form, [data-product-form]"):document.body}}function Dt(s,t){switch(s){case"chat":return new bt(t);case"toast":return new Pt(t);case"countdown_banner":return new St(t);case"inline_alert":return new Tt(t);case"bundle":return new wt(t);case"upsell_drawer":return new $t(t);case"discount_nudge":return new kt(t);case"rec_strip":return new Ct(t);case"social_proof":return new At(t);case"exit_intent":return new xt(t);case"post_purchase":return new It(t);default:return null}}function S(s){const t=document.querySelector(s),e=document.createElement("div");return e.setAttribute("data-aovboost-mount",s),t?.parentElement?(t.insertAdjacentElement("afterend",e),e):(document.body.appendChild(e),e)}let N=!1;function P(){N||(N=!0,Mt().catch(s=>{console.log("AOVBoost SDK skipped:",s instanceof Error?s.message:String(s))}))}async function Mt(){try{const s=window.AOVBoost||{},t=s.shop;if(!t)return;j(s)||await qt(s);const e=Vt(s.apiBase),i=new at(t,e),o=new m({shop:t,sessionManager:i,apiBase:e}),a=new Lt,d=new et({shop:t,apiBase:e,eventBus:o,sessionManager:i,widgetManager:a}),c=new nt({eventBus:o,offerPoller:d,sessionManager:i});await i.init(),window.AOVBoostSDK={shop:t,sessionId:i.anonymousId,sessionToken:i.getAuthPayload().sessionToken,refreshSession:async()=>{await i.refreshAuth(),i.syncGlobalSdkAuth()},track:(u,h={})=>o.track(u,h),trigger:(u,h={})=>c.trigger(u,h),requestOffer:(u="global",h={})=>d.requestOffer(u,h),destroy:()=>{c.destroy(),d.destroy(),i.destroy(),a.destroyActive()}},c.init(),o.init(),d.init()}catch(s){console.log("AOVBoost SDK skipped:",s instanceof Error?s.message:String(s))}}function Vt(s){const t=typeof s=="string"?s.trim():"";return!t||t==="/api"||t.startsWith("/api/")?"/apps/aovboost":t.includes("/apps/aovboost")||t.startsWith("/apps/")?t:"/apps/aovboost"}function j(s){if(s.settings?.trackingConsentRequired!==!0)return!0;const t=window.Shopify?.customerPrivacy;return typeof t?.analyticsProcessingAllowed=="function"?!!t.analyticsProcessingAllowed():typeof t?.userCanBeTracked=="function"?!!t.userCanBeTracked():!0}function qt(s){return new Promise(t=>{const e=()=>{j({...s,settings:{...s.settings,trackingConsentRequired:!1}})&&(i(),t())},i=()=>{["visitorConsentCollected","shopify:customer_privacy:consent_collected","aovboost:consent-granted"].forEach(o=>window.removeEventListener(o,e))};["visitorConsentCollected","shopify:customer_privacy:consent_collected","aovboost:consent-granted"].forEach(o=>window.addEventListener(o,e))})}return document.readyState==="loading"?document.addEventListener("DOMContentLoaded",P,{once:!0}):P(),f.init=P,Object.defineProperty(f,Symbol.toStringTag,{value:"Module"}),f}({});
